<template>
  <div class="card mt-3">
    <div class="card-header">
      <h3 class="card-title">Tabulador de Ponles</h3>
    </div>
    <div class="table-responsive pt-4">
      <vue-bootstrap4-table
        :rows="ponles.data ? ponles.data : []"
        :columns="columns"
        :config="config"
        @on-change-query="onChangeQuery"
        :show-loader="loading"
        :totalRows="ponles.total">
      <template slot="empty-results">
        No hay ponle registrados
      </template>
...
export default {
  ...
  computed: {
    ...
    loading: state => state.ponles.loading
  }),
  },
   watch: {
    'type': function (val) {
  ...
  methods: {
    ...
    async deleteItem(item) {
      let me = this;
      const index = this.ponles.data.indexOf(item);
      if (
        confirm("Esta seguro que desea eliminar?") &&
        this.ponles.data.splice(index, 1)
      ) {
        const data = await this.$store.dispatch("ponles/delete", item);
        if (data) {
          me.$toastr.success(
